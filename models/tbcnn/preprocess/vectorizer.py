...
def learn_vectors(samples, output_path, logdir, log_file,
                  num_features, batch_size, hidden_size,
                  learn_rate, epochs, checkpoint_step):
    NODE_MAP = _create_node_map(samples)
    NODE_LIST = list(NODE_MAP.keys())
    # build the inputs and outputs of the network
    input_node, label_node, embed_node, loss_node = _init_net(num_features, batch_size, hidden_size, len(NODE_LIST))
...
def _create_node_map(samples):
    nodes = list(set(samples['node'].tolist()))
